...
| Media probe + ffmpeg remux/convert (AVIF?ebP) | ??|
...
| eb29056 | **Correctness** | `get_galleries` no longer builds SQL with `format!()` ??each allowed ORDER BY is a compile-time `&'static str` chosen via `match`, satisfying the SQL safety rule in `CONTRIBUTING.md`. `long_strip` reading mode is now handled (shares `vertical_scroll` branch). |
...
- **Any feature gated behind "drive connected" without an explicit offline path** ??see `CONTRIBUTING.md`.
...
Notable bug-fix background is preserved in `docs/BUG_FIX_GUIDE.md`. Treat the current source code and this status document as authoritative for the present code shape.
