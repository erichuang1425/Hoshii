...
interface TagModalProps {
  ...
  onClose: () => void;
  tags: Tag[];
  onFetchTags: (galleryId: number) => void;
  onAddTag: (galleryId: number, name: string) => Promise<void>;
  onRemoveTag: (galleryId: number, tagId: number) => Promise<void>;
}
...
export function TagModal({
  ...
  useEffect(() => {
    if (open) onFetchTags(galleryId);
  }, [open, galleryId, onFetchTags]);
  ...
  async function handleRemoveTag(tagId: number) {
    try {
      await onRemoveTag(galleryId, tagId);
